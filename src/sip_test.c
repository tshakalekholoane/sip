...
static const uint128_t key = 0x0f0e0d0c0b0a09080706050403020100wbu;
...
int main(void) {
  uint64_t got;
  for (size_t data_len = 0; data_len < 64; data_len += 1) {
    got = sip_hash(key, data, data_len);
    if (got != want[data_len]) [[clang::unlikely]] {
      (void) fprintf(
        stderr, "got sip_hash(key, data, data_len = %zu) = %llx, want %llx\n",
        data_len, got, want[data_len]
      );
      return EXIT_FAILURE;
    }
  }
